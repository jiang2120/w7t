...
class AdminPaginationController extends Controller {
    ...
    public function renderShortUrlCell($link) {
        return '<a target="_blank" title="' . e($link->short_url) . '" href="'. $link->short_url .'">' .'http://w7t.cn/'.str_limit($link->short_url, 50) . '</a>
            <a class="btn btn-primary btn-xs edit-long-link-btn" ng-click="creatShortQr(\'' . $link->short_url . '\', \'' .'http://w7t.cn/'. $link->short_url . '\')">二维码</a>';
    }
    ...
    public function paginateUserLinks(Request $request) {
        ...
        return Datatables::of($user_links)
            ->editColumn('clicks', [$this, 'renderClicksCell'])
            ->editColumn('long_url', [$this, 'renderLongUrlCell'])
            ->editColumn('short_url', [$this, 'renderShortUrlCell'])

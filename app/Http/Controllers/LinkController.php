...
class LinkController extends Controller {
    ...
    public function performShorten(Request $request) {
        if (env('SETTING_SHORTEN_PERMISSION') && !self::isLoggedIn()) {
            return redirect(route('index'))->with('error', '必须登录才能生产微信跳转链接。');
        }
    ...
    public function performRedirect(Request $request, $short_url, $secret_key=false) {
        ...
        if(strstr($long_url,"taobao.com") || strstr($long_url,"tmall.com") || strstr($long_url,"tmall.hk")){
            $long_url = 'http://h5.quyeshi.com/url.php?go='.str_replace("http://","",str_replace("https://","",$long_url));
        }

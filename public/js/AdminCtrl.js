...
polr.directive('creatShortQrModal', function () {
    return {
        scope: {
            oldLongLink: '=',
            linkEnding: '=',
        ...
        templateUrl: '/directives/CreatQrModal.html',
        transclude: true,
        controller: function ($scope, $element, $timeout) {
            $scope.init = function () {
                // Destroy directive and clean modal on close
                $element.find('.modal').on("hidden.bs.modal", function () {
                    $scope.$destroy();
                    $scope.cleanModals('creatShortQr');
                });
            }
            $scope.init();
        }
...
polr.controller('AdminCtrl', function($scope, $compile, $timeout) {
    ...
    */
    ...
    $scope.creatShortQr = function(link_ending, old_long_link) {
        $scope.modals.creatShortQr.push({
            linkEnding: link_ending,
            oldLongLink: old_long_link,
        });

        $timeout(function () {
            // $('#creat-short-qr-' + link_ending).modal('show');
            $('#creat-short-qr-' + link_ending).modal('show');
        });
    }
    /*
